...
frame_queue fr_q[NFRAMES];
int circ_q_len;

//frame_queue circ_q[NFRAMES];
/************************************************************************/
/***				NOTE:				      ***/
/***								      ***/
...
sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	/*end of SC policy implementation*/
	
	/*setting up one to one map here*/
	pt_t *pt_entry;
	for(i=0; i<4; i++)
	{	
		...
		for(j=0; j<1024; j++, pt_entry++)
		{
			pt_entry->pt_pres = 1;
                        pt_entry->pt_write = 1;
                        pt_entry->pt_base = i*1024 + j;
		}
	}
	create_pd(NULLPROC);
	set_evec(14, pfintr);
